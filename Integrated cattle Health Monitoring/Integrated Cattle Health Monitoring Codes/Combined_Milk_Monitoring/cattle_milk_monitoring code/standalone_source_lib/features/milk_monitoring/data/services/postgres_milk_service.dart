...
class PostgresMilkService implements MilkRepository {
  ...
  PostgresMilkService({
    this.baseUrl = 'http://54.144.103.253:8082',
  ...
  Future<void> _fetchRecords() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        MockMilkService.syncFromCloud(records);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return MilkAnalyticsModel(
          dailyTotal: (json['daily_total'] as num?)?.toDouble() ?? 0.0,
          morningMilk: (json['morning_total'] as num?)?.toDouble() ?? 0.0,
          eveningMilk: (json['evening_total'] as num?)?.toDouble() ?? 0.0,
          bestDay: 'Thursday',
          avgDaily: ((json['daily_total'] as num?)?.toDouble() ?? 0.0) / 10.0,
          weeklyTotal: (json['daily_total'] as num?)?.toDouble() ?? 0.0,
          monthlyTotal: ((json['daily_total'] as num?)?.toDouble() ?? 0.0) * 3.0,
        );
      }
    } catch (e) {
      debugPrint('Error fetching analytics from AWS: $e');
    } finally {
      client.close();
    }
  ...
  Future<void> updateMilkAnalytics(MilkAnalyticsModel analytics) async {
    // Cloud analytics are computed server-side
  }
