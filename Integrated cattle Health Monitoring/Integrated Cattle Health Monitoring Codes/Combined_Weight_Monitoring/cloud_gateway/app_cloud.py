...
def get_history():
    ...
    # Also fetch completed CloudTasks for this user to guarantee 100% complete history persistence
    completed_tasks = CloudTask.query.filter_by(user_id=user_id, status="completed").order_by(CloudTask.completed_at.desc()).all()
    for t in completed_tasks:
        if str(t.id) in seen_ids:
            continue
        glb_url = f"/download/3d/{t.id}/model.glb" if t.glb_filename and "dummy" not in t.glb_filename else None
        if s3_client and S3_BUCKET_NAME and glb_url:
            filename = f"{t.id}_model.glb"
            glb_url = get_file_url(filename)
            
        ts_str = (t.completed_at or t.created_at).strftime("%Y-%m-%d %H:%M:%S") if (t.completed_at or t.created_at) else ""
        history_list.append({
            "id": str(t.id),
            "cow_id": t.cow_id or 'UNKNOWN',
            "name": t.cow_name or 'Cattle',
            "timestamp": ts_str,
            "weight_kg": t.weight_kg or 0.0,
            "measurements": json.loads(t.measurements_json) if t.measurements_json else {},
            "section": t.section or 'General',
            "breed": t.breed or 'Unknown',
            "glb_url": glb_url
        })
...
def worker_complete_task(task_id):
    ...
    try:
        ...
        if weight_kg is None and result_json:
            try:
                res_obj = json.loads(result_json)
                weight_kg = float(res_obj.get("predicted_weight_kg", 0.0))
            except Exception:
                weight_kg = 0.0
        ...
        record = CattleRecord(
            cattle_id=task.cow_id or f"Cattle-{task_id[:6]}",
            name=task.cow_name or f"Cow-{task_id[:6]}",
            breed=task.breed or "Gir",
            section=task.section or "Dairy Cattle",
            weight_kg=weight_kg,
        ...
        print(f"[WORKER COMPLETE] Task {task_id} successfully finalized with weight: {weight_kg} kg")
