...
class VaccinationReportScreen extends StatefulWidget {
  ...
  const VaccinationReportScreen({
    Key? key,
    this.cattleId = 'KA-1989',
    this.cattleName = 'Geetha',
  }) : super(key: key);
...
class _VaccinationReportScreenState extends State<VaccinationReportScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Focus on target cattle data (dynamic fallback to registered herd if default not found)
    ...
    if (!registry.isCattleRegistered(targetId, targetName) && registry.cattles.isNotEmpty) {
      final firstCattle = registry.cattles.first;
      targetId = firstCattle.cattleId;
      targetName = firstCattle.name;
    }
