...
class MilkProvider extends ChangeNotifier {
  ...
  void _recalculateLocalAnalytics() {
    ...
    // If no milk logged today yet, get the most recent active milking date from records
    if (dailyTotal == 0.0) {
      final sortedRecords = List<MilkRecordModel>.from(_records)
        ..sort((a, b) => b.timestamp.compareTo(a.timestamp));
      if (sortedRecords.isNotEmpty) {
        final latestDateStr = localDateOf(sortedRecords.first.timestamp);
        for (final r in sortedRecords) {
          if (localDateOf(r.timestamp) == latestDateStr) {
            dailyTotal += r.quantity;
            if (r.milkTime.toLowerCase() == 'morning') {
              morningTotal += r.quantity;
            } else if (r.milkTime.toLowerCase() == 'evening') {
    ...
    // Daily totals map for analytics
    final dailyTotals = <String, double>{};
    for (final r in _records) {
      final dateStr = localDateOf(r.timestamp);
    ...
    _analytics = MilkAnalyticsModel(
      dailyTotal: dailyTotal > 0 ? dailyTotal : _analytics.dailyTotal,
      ...
      weeklyTotal: weeklyTotal > 0 ? weeklyTotal : _analytics.weeklyTotal,
      monthlyTotal: monthlyTotal > 0 ? monthlyTotal : _analytics.monthlyTotal,
    );
  }
