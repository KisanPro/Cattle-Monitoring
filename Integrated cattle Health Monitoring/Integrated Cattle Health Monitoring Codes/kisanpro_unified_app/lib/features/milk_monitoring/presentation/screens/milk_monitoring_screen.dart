...
import '../../../auth/providers/farmer_auth_provider.dart';
...
class _MilkMonitoringScreenState extends State<MilkMonitoringScreen> {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected ? AppTheme.primary : const Color(0xFFEBEFEF),
          borderRadius: BorderRadius.circular(20),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(icon, size: 13, color: isSelected ? Colors.white : AppTheme.textMuted),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Column(
        children: [
          // ── Top Sub-Nav Strip with Chips & Cloud Sync ──
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            decoration: BoxDecoration(
              color: isDark ? AppTheme.surfaceDark : Colors.white,
              border: const Border(bottom: BorderSide(color: AppTheme.borderLight, width: 1)),
            ),
            child: Row(
              children: [
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: Row(
                      children: [
                        _buildTabChip(0, Icons.dashboard_outlined, "Dashboard"),
                        const SizedBox(width: 6),
                        _buildTabChip(1, Icons.analytics_outlined, "Analytics"),
                        const SizedBox(width: 6),
                        _buildTabChip(2, Icons.history_outlined, "Logbook"),
                        const SizedBox(width: 6),
                        _buildTabChip(3, Icons.picture_as_pdf_outlined, "Reports"),
                      ],
                    ),
                  ),
                ),
                          ...
                          SnackBar(
                            content: Text(
                              provider.isMockMode 
                                  ...
                                  : "Live Cloud PostgreSQL Synchronization active.",
                            ),
...
class _DashboardTabState extends State<DashboardTab> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<MilkProvider>(
      builder: (context, provider, child) {
        ...
        final recentGroups = provider.groupRecordsByCattleAndDay(records).toList();
  ...
  Widget _buildFarmerHeader(bool isDark) {
    ...
    final farmerName = context.watch<FarmerAuthProvider>().farmerName;
