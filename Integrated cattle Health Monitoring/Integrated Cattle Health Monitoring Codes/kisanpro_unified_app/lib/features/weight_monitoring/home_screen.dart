...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void didChangeDependencies() {
    ...
    try {
      ...
      if (_lastCattleId != currentCattle.cattleId) {
        ...
        _cowIdController.text = currentCattle.cattleId;
        _cowNameController.text = currentCattle.name;
        for (var b in _breeds) {
          if (currentCattle.breed.toLowerCase().contains(b.toLowerCase())) {
            _selectedBreed = b;
            break;
          }
        }
  ...
  Future<void> _loadHistoryAndAlerts() async {
    try {
      ...
      final registry = mounted ? context.read<CattleRegistryProvider>() : null;

      // Filter list so only registered cattle records are kept
      final filteredList = list.where((r) {
        if (registry == null) return true;
        final cowId = (r['cow_id'] ?? '').toString();
        final cowName = (r['cow_name'] ?? r['name'] ?? '').toString();
        return registry.isCattleRegistered(cowId, cowName);
      }).toList();
      ...
      // Auto-synchronize latest weights into the unified CattleRegistryProvider
      for (var entry in grouped.entries) {
        final cowId = entry.key;
        final cowList = entry.value;
        if (cowList.isNotEmpty) {
          final double? latestW = (cowList.last['weight_kg'] ?? cowList.last['predicted_weight_kg'] as num?)?.toDouble();
          if (latestW != null && latestW > 0 && mounted) {
            try {
              context.read<CattleRegistryProvider>().updateCattleTelemetry(
                cattleId: cowId,
                weight: latestW,
              );
            } catch (_) {}
      ...
      setState(() {
        _historyList = filteredList.map((item) => Map<String, dynamic>.from(item)).toList();
        _alertCount = count;
      });
    } catch (e) {
  ...
  void _submitForm() async {
    ...
    try {
      ...
      // Step B: Poll task status with robust error recovery
      ...
      while (!isFinished) {
        ...
        try {
          ...
          // If server reported task failure explicitly, rethrow immediately
          if (pollErr.toString().contains('Task execution failed on server') ||
              pollErr.toString().contains('Task not found')) {
            rethrow;
          }
          consecutiveErrors++;
          // Allow transient socket/network drops up to 10 consecutive times (~20-25s grace period)
          if (consecutiveErrors > 10) {
            throw Exception('Network connection lost while waiting for AI results. Please check internet connection.');
          }
        }
      }
      ...
      // Auto-update Cattle Registry telemetry immediately
      final double? predWeight = (finalResult?['predicted_weight_kg'] as num?)?.toDouble();
      if (predWeight != null && predWeight > 0 && mounted) {
        try {
          context.read<CattleRegistryProvider>().updateCattleTelemetry(
            cattleId: _cowIdController.text.trim(),
            weight: predWeight,
          );
        } catch (_) {}
      }
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: Colors.white,
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: const Color(0xFF006D60).withOpacity(0.2)),
                  boxShadow: [
                    BoxShadow(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.monitor_weight_outlined, color: Color(0xFF006D60), size: 22),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        _username != null && _username!.isNotEmpty
                            ? 'Weight Estimation ($_username)'
                            : '3D Weight & Mesh Estimation',
                        style: const TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.bold,
                          color: Color(0xFF006D60),
                    ...
                    Stack(
                      ...
                      children: [
                        IconButton(
                          icon: const Icon(Icons.notifications_none, color: Color(0xFF006D60), size: 20),
                          tooltip: 'Alerts',
                          onPressed: () {
                            Navigator.push(
                              context,
                    ...
                    IconButton(
                      icon: const Icon(Icons.verified_outlined, color: Color(0xFF006D60), size: 20),
                      tooltip: 'Validation',
                      onPressed: () {
                        Navigator.push(
                          context,
                    ...
                    IconButton(
                      icon: const Icon(Icons.history, color: Color(0xFF006D60), size: 20),
                      tooltip: 'History',
                      onPressed: () {
                        Navigator.push(
                          context,
                    ...
                    IconButton(
                      icon: const Icon(Icons.settings_outlined, color: Color(0xFF006D60), size: 20),
