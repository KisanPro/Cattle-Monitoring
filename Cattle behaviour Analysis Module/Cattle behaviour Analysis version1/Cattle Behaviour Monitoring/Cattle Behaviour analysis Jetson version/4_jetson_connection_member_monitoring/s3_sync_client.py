...
KISAN_JETSON_DIR = os.path.join(os.path.dirname(CURRENT_DIR), "Kisan_Jetson")
EMBEDDINGS_DIR = os.path.join(KISAN_JETSON_DIR, "embeddings")
MODELS_DIR = os.path.join(KISAN_JETSON_DIR, "models")
...
def perform_sync():
    try:
        ...
        try:
            ...
            if "404" in str(e) or "Not Found" in str(e):
                if os.path.exists(local_pkl):
                    print(f"[Jetson Cloud Sync] S3 embeddings not found. Uploading local {local_pkl} to S3...")
                    s3.upload_file(local_pkl, AWS_S3_BUCKET, s3_pkl_key)
                else:
                    raise e
            else:
                raise e
        ...
        try:
            ...
            if "404" in str(e) or "Not Found" in str(e):
                if os.path.exists(local_roles):
                    print(f"[Jetson Cloud Sync] S3 roles not found. Uploading local {local_roles} to S3...")
                    s3.upload_file(local_roles, AWS_S3_BUCKET, s3_roles_key)
                else:
                    raise e
            else:
                raise e
        ...
        try:
            ...
            if "404" in str(ck_err) or "Not Found" in str(ck_err):
                if os.path.exists(local_ckpt):
                    print(f"[Jetson Cloud Sync] S3 checkpoint not found. Uploading local {local_ckpt} to S3...")
                    s3.upload_file(local_ckpt, AWS_S3_BUCKET, s3_ckpt_key)
                else:
                    print(f"[Jetson Cloud Sync] Warning: Checkpoint file not found on S3 or locally.")
            else:
