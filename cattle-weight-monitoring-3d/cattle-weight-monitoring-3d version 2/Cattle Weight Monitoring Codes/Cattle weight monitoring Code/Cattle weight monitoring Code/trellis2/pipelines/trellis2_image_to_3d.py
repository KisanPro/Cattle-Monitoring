...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    @classmethod
    def from_pretrained(cls, path: str, config_file: str = "pipeline.json", dtype: torch.dtype = None) -> "Trellis2ImageTo3DPipeline":
        """
        ...
        """
        pipeline = super().from_pretrained(path, config_file, dtype=dtype)
    ...
    def preprocess_image(self, input: Union[Image.Image, List[Image.Image]]) -> Union[Image.Image, List[Image.Image]]:
        """
        Preprocess the input image(s).
        """
        # Proactively load the background segmentation model to GPU for ultra-fast inference
        if self.rembg_model is not None:
            self.rembg_model.cuda()
            
        try:
            if isinstance(input, list):
                return [self._preprocess_single_image(img) for img in input]
            else:
                return self._preprocess_single_image(input)
        finally:
            # Immediately offload from GPU and completely reclaim the VRAM
            if self.rembg_model is not None:
                self.rembg_model.cpu()
            torch.cuda.empty_cache()

    def _preprocess_single_image(self, input: Image.Image) -> Image.Image:
        """
        Perform preprocessing on a single PIL image.
        """
        ...
        output = Image.fromarray(output_np).crop(bbox)
        return output
    ...
    def get_cond(self, image: Union[torch.Tensor, list[Image.Image]], resolution: int, include_neg_cond: bool = True) -> dict:
        ...
        if self.low_vram and next(self.image_cond_model.parameters()).device.type == 'cpu':
            self.image_cond_model.to(self.device)
            
        cond = self.image_cond_model(image)
        
        # Move back to main pipeline device if processed on a secondary GPU
        cond = cond.to(self.device)
        
        if cond.ndim == 3 and cond.shape[0] > 1:
            # Reshape from (V, L, D) to (1, V * L, D) to preserve all perspective features
    ...
    def sample_sparse_structure(
        ...
        noise = torch.randn(num_samples, in_channels, reso, reso, reso, dtype=self.dtype, device=self.device)
    ...
    def sample_shape_slat(
        ...
        noise = SparseTensor(
            feats=torch.randn(coords.shape[0], flow_model.in_channels, dtype=self.dtype, device=self.device),
            coords=coords,
        )
    ...
    def sample_shape_slat_cascade(
        ...
        # LR
        noise = SparseTensor(
            feats=torch.randn(coords.shape[0], flow_model_lr.in_channels, dtype=self.dtype, device=self.device),
            coords=coords,
        )
        sampler_params = {**self.shape_slat_sampler_params, **sampler_params}
        if self.low_vram:
    ...
    def sample_tex_slat(
        ...
        noise = shape_slat.replace(feats=torch.randn(shape_slat.coords.shape[0], in_channels - shape_slat.feats.shape[1], dtype=self.dtype, device=self.device))
    ...
    def run(
        ...
        Args:
            image (Union[Image.Image, List[Image.Image]]): The image prompt(s). Provide a list for multi-view.
            num_samples (int): The number of samples to generate.
