...
class ResultScreen extends StatelessWidget {
  ...
  // Convert cm to inches or keep as cm
  double _convertLength(double cm) {
    return isImperial ? cm * 0.393701 : cm;
  }

  String get _weightUnit => isImperial ? 'lbs' : 'kg';
  String get _lengthUnit => isImperial ? 'in' : 'cm';
  ...
  Widget build(BuildContext context) {
    ...
    final Map<String, dynamic> measurements = result['measurements'] ?? {};
    final double obl = (measurements['OBL_cm'] as num? ?? 0.0).toDouble();
    final double wh = (measurements['WH_cm'] as num? ?? 0.0).toDouble();
    final double hg = (measurements['HG_cm'] as num? ?? 0.0).toDouble();
    final double hl = (measurements['HL_cm'] as num? ?? 0.0).toDouble();
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Cattle ID: $cowId • Category: $section',
  ...
  Widget _buildDimensionTile(String name, double value) {
    return Card(
      elevation: 0.5,
      color: Colors.white,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              '${_convertLength(value).toStringAsFixed(1)} $_lengthUnit',
              style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: Color(0xFF006D60)),
            ),
          ],
        ),
      ),
