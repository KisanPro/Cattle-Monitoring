...
class Pipeline:
    ...
    @classmethod
    def from_pretrained(cls, path: str, config_file: str = "pipeline.json", dtype: torch.dtype = None) -> "Pipeline":
        """
        ...
        for k, v in args['models'].items():
            if hasattr(cls, 'model_names_to_load') and k not in cls.model_names_to_load:
                continue
            try:
                _models[k] = models.from_pretrained(f"{path}/{v}", dtype=dtype)
            except Exception as e:
                _models[k] = models.from_pretrained(v, dtype=dtype)
