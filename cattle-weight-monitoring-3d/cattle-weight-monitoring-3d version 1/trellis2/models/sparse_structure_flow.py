...
class SparseStructureFlowModel(nn.Module):
    ...
    def convert_to(self, dtype: torch.dtype) -> None:
        ...
        """
        self.dtype = dtype
        self.input_layer.to(dtype=dtype)
        self.out_layer.to(dtype=dtype)
        self.t_embedder.to(dtype=dtype)
        if hasattr(self, 'adaLN_modulation'):
            self.adaLN_modulation.to(dtype=dtype)
        if hasattr(self, 'pos_emb') and self.pos_emb is not None:
            self.pos_emb = self.pos_emb.to(dtype=dtype)
