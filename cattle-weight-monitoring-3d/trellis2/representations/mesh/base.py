...
try:
    ...
    cumesh = None
    print("Warning: cumesh not found. Mesh simplification and hole filling will be disabled.", flush=True)
...
class Mesh:
    ...
    def fill_holes(self, max_hole_perimeter=3e-2):
        if cumesh is None:
            print("Warning: cumesh not installed. Skipping fill_holes.")
            return
        ...
        try:
            ...
            self.vertices = new_vertices.to(self.device).to(self.vertices.dtype)
            self.faces = new_faces.to(self.device)
        except RuntimeError as e:
            print(f"Warning: cumesh fill_holes failed (OOM?), skipping. {e}", flush=True)
        
    def remove_faces(self, face_mask: torch.Tensor):
        if cumesh is None:
            print("Warning: cumesh not installed. Skipping remove_faces.")
            return
        ...
        try:
            ...
            print(f"Warning: cumesh remove_faces failed (OOM?), skipping. {e}", flush=True)
        
    def simplify(self, target=1000000, verbose: bool=False, options: dict={}):
        if cumesh is None:
            print("Warning: cumesh not installed. Skipping simplify.")
            return
        ...
        try:
            ...
            print(f"Warning: cumesh simplify failed (OOM?), skipping. {e}", flush=True)
